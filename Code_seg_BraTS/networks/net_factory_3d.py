...
def net_factory_3d(net_type="unet_3D", in_chns=1, class_num=2, device=0):
    if net_type == "unet_3D":
        net = unet_3D(n_classes=class_num, in_channels=in_chns).to(device)
    elif net_type == "trinet":
        net = TriNet(n_classes=class_num, in_channels=in_chns).to(device)
    elif net_type == "BiNet":
        net = BiNet(n_classes=class_num, in_channels=in_chns).to(device)
