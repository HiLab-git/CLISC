...
class BaseDataSets(Dataset):
    def __init__(self, base_dir=None, split='train', transform=None, fold="fold1", sup_type="label"):
        ...
        if self.split == 'train':
            ...
            self.all_volumes = os.listdir(self._base_dir + "/valid")
            self.sample_list = []
            for ids in test_ids:
...
class BaseDatasets_fetal(Dataset):
    def __init__(self, base_dir=None, split='train', transform=None, sup='label'):
        self._base_dir = base_dir
        self.sample_list = []
        self.split = split
        self.transform = transform
        self.sample_list = os.listdir(self._base_dir + "/" + self.split)
        self.sup = sup
    ...
    def __getitem__(self, idx):
        ...
        if self.split == 'train':
            image = load_image_as_nd_array(filename)['data_array'].squeeze()
            label = load_image_as_nd_array(os.path.join(os.path.dirname(self._base_dir), self.sup, self.split, case))['data_array'].squeeze()
            sample = {'image': image, 'label': label}
            ...
            image = load_image_as_nd_array(filename)['data_array'].squeeze(axis=0)
            label = load_image_as_nd_array(os.path.join(os.path.dirname(self._base_dir), self.sup, self.split, case))['data_array'].squeeze(axis=0)
            image = image / 1.0
            label = label / 1.0
