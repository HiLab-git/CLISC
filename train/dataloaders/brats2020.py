...
class BraTS2020(Dataset):
    def __init__(self, csv_path=None, exp=None, transform=None):
    ...
    def __getitem__(self, idx):
        ...
        if self.exp == "SAM_Sup":
            label_path = filename.replace("image", "SAM_label")
        elif self.exp == "UNet_Sup":
            label_path = filename.replace("image", "UNet_label")
        else:
            label_path = filename.replace("image", "label")
        
        label = sitk.GetArrayFromImage(sitk.ReadImage(label_path)).squeeze()
